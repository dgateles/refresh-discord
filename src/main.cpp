...
fs::path Config(const Proxy&p){fs::path root=Root(),logs=root/L"logs";fs::create_directories(logs);fs::path path=root/(L"session-"+std::to_wstring(GetTickCount64())+L".json");std::ostringstream j;
 j<<R"({"log":{"level":"info","output":")"<<Utf8((logs/L"session.log").wstring())<<R"(","timestamp":true},)"<<R"("dns":{"servers":[{"type":"local","tag":"dns-local"}],"final":"dns-local","strategy":"prefer_ipv4"},)"<<R"("inbounds":[{"type":"tun","tag":"tun-in","interface_name":"DiscordRefresh","address":["172.20.0.1/30"],"mtu":1408,"auto_route":true,"strict_route":true,"stack":"mixed"}],)"<<R"("outbounds":[{"type":"direct","tag":"direct"},{"type":"http","tag":"temp-proxy","server":")"<<Utf8(p.host)<<R"(","server_port":)"<<p.port<<R"(}],)"<<R"("route":{"auto_detect_interface":true,"default_domain_resolver":"dns-local","rules":[{"action":"sniff"},{"protocol":"dns","action":"hijack-dns"},{"ip_is_private":true,"action":"route","outbound":"direct"},{"process_name":["Discord.exe","DiscordCanary.exe","DiscordPTB.exe","DiscordDevelopment.exe"],"network":"tcp","action":"route","outbound":"temp-proxy"}],"final":"direct"}})";std::ofstream(path,std::ios::binary)<<j.str();return path;}
